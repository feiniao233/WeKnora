...
func (t *SearchKnowledgeTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	result.KnowledgeReferences = make([]*types.SearchResult, 0, len(final))
	for _, item := range final {
		if item != nil && item.SearchResult != nil {
			result.KnowledgeReferences = append(result.KnowledgeReferences, item.SearchResult)
		}
	}
	return result, nil
}
