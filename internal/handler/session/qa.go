...
func (h *Handler) executeQA(reqCtx *qaRequestContext, mode qaMode, generateTitle bool) {
	...
	if mode == qaModeNormal {
		...
		// A streaming failure may arrive without a final-answer Done marker.
		// Persist the classified outcome before releasing the deferred completion.
		streamCtx.eventBus.On(event.EventError, func(ctx context.Context, evt event.Event) error {
			result := executionResult(streamCtx.asyncCtx)
			if completionHandled || result == nil || result.Status != "failed" {
				return nil
			}
			completionHandled = true
			updateCtx := context.WithValue(
				context.WithoutCancel(streamCtx.asyncCtx), types.TenantIDContextKey, reqCtx.session.TenantID,
			)
			h.completeQuickAnswerTurn(updateCtx, streamCtx, "", "")
			return nil
		})
	}
